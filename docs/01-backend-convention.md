...
| Kafka Event | `{대상}{행위}Event` | `OrderConfirmedEvent`, `CreditApprovedEvent` |
...
- ID 필드: `{대상}Id` → `userId`, `creditLimitId`
...
public ApiResponse<UserResponse> getMe(@AuthUser AuthUserInfo authUser) {
    return ApiResponse.success(userService.getUser(authUser.userId()));
}
```
...
- 이벤트 페이로드에 내부 `id` 사용 가능 (서비스 간 내부 통신이므로)
...
public void handleOrderConfirmed(OrderConfirmedEvent event) {
    if (creditUsageLedgerRepository.existsByOrderId(event.orderId())) {
        return;
    }
