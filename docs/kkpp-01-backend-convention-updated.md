...
| 존재 확인 | `exists` | `existsByPhone()`, `existsByOrderPublicId()` |
| 생성 | `create` | `createOrder()` |
| 수정 | `update` | `updatePinHash()` |
| 삭제/취소 | `cancel`, `delete` | `cancelOrder()` |
| 검증 | `validate` | `validatePin()` |
| Repository 쿼리 | Spring Data 네이밍 규칙 | `findByUserPublicId()`, `findAllByStatus()`, `existsByOrderPublicId()` |

### 1.3 변수 / 필드

- camelCase 사용
- Boolean 필드: `is` 접두사 → `isRead`, `isReapplication`
- 각 테이블의 내부 PK는 `id`로 두며, 타입은 `Long`을 사용한다.
- 각 테이블의 외부/API/서비스 간 식별자는 `publicId(UUID)`를 사용한다.
- 외부 응답, Kafka 이벤트, Redis key, 서비스 간 참조에는 내부 `Long id`를 노출하지 않는다.
- 테이블 간 참조 컬럼은 기본적으로 참조 대상 테이블의 `public_id(UUID)`를 사용한다.
- DB 컬럼명은 `user_public_id`, `application_public_id`, `credit_limit_public_id`처럼 작성한다.
- Entity 필드명도 DB 컬럼에 맞춰 `{대상}PublicId` 형태로 작성한다.
- 내부 id 기반 FK 필드인 `userId`, `creditLimitId`, `orderId` 등은 새로 추가하지 않는다.
- 단, 자기 테이블의 PK인 `id`는 `Long`으로 유지한다.
- 단일 서비스 내부에서 이미 조회된 Entity의 내부 PK를 임시로 사용할 수는 있지만, DB 저장 컬럼이나 서비스 간 이벤트에는 `publicId(UUID)`를 사용한다.
...
| 구분 | DB 컬럼 | Entity 필드 |
|------|---------|-------------|
| 사용자 참조 | `user_public_id` | `userPublicId` |
| 한도 신청 참조 | `application_public_id` | `applicationPublicId` |
| 한도 참조 | `credit_limit_public_id` | `creditLimitPublicId` |
| 주문 참조 | `order_public_id` | `orderPublicId` |
| 결제요청 참조 | `payment_request_public_id` | `paymentRequestPublicId` |
| 지갑 참조 | `wallet_public_id` | `walletPublicId` |
...
- Repository에서 `Optional`을 반환하고 Service에서 `.orElseThrow()`를 사용한다.
...
- `@Setter` 클래스 레벨 사용 금지 → 상태 변경은 의미 있는 메서드로 처리
- `FetchType.EAGER` 사용 금지 → 항상 `LAZY`
- `@Data` 사용 금지 → `@Getter` + 필요한 어노테이션만 사용
- 양방향 연관관계는 꼭 필요할 때만 사용 → 단방향 우선
- 다른 서비스 소유 Entity/Repository 직접 import 금지 → publicId 저장 또는 HTTP/Kafka 연동 사용
- DB 저장 컬럼에 `user_id`, `application_id`, `credit_limit_id`, `order_id` 같은 내부 id 기반 FK를 새로 추가하는 방식 금지

### 4.3 BaseEntity 상속 기준

| 구분 | 상속 대상 | 해당 테이블 |
|------|-----------|-------------|
| `BaseEntity` | `created_at` + `updated_at` | users, user_auth, farmer_profiles, farmer_documents, credit_limit_applications, credit_limits, products, orders 등 |
| `BaseTimeEntity` | `created_at` 만 | audit_logs, wallet_transactions, order_items, ass_scores, bss_scores, credit_usage_ledger 등 |

### 4.4 Entity 식별자 매핑 규칙

Entity 작성 시 다음 기준을 우선한다.

- `id`는 해당 테이블 내부 PK로만 사용한다.
- `publicId`는 외부 API, Kafka 이벤트, Redis key, 서비스 간 참조에 사용하는 공개 식별자이다.
- FK 성격의 컬럼은 기본적으로 참조 대상 테이블의 `public_id`를 저장한다.
- DB 컬럼명은 `user_public_id`, `application_public_id`, `credit_limit_public_id`, `order_public_id`처럼 작성한다.
- Entity 필드명은 `userPublicId`, `applicationPublicId`, `creditLimitPublicId`, `orderPublicId`처럼 작성한다.
- `Long userId`, `Long creditLimitId`, `Long orderId` 같은 내부 id 기반 FK 필드는 새로 추가하지 않는다.
- 단일 서비스 내부에서 이미 조회된 Entity의 내부 PK를 임시로 사용할 수는 있지만, DB 저장 컬럼이나 서비스 간 이벤트에는 publicId를 사용한다.
...
연관관계를 사용하는 경우에도 JoinColumn은 public_id 기준으로 작성한다.
...
- Entity → Response DTO 변환은 MapStruct 사용을 기본으로 한다.
- Controller나 Service에서 직접 `new ResponseDto(entity.getField())` 방식은 지양한다.
- 단순 응답이거나 MVP 범위에서 Mapper 도입 비용이 큰 경우에는 팀 합의 후 직접 변환을 허용할 수 있다.
...
JWT 내부 claim에는 구현 편의상 내부 `userId`가 포함될 수 있다.  
다만 외부 API 응답, Kafka 이벤트, Redis key, 서비스 간 연동에서는 `userPublicId`를 사용한다.
...
- 이벤트 클래스는 `common-core`의 event 패키지에 위치한다.
- Producer는 트랜잭션 커밋 후 발행한다.
- 트랜잭션 커밋 후 이벤트 발행에는 `@TransactionalEventListener`를 사용한다.
- Consumer는 멱등성을 보장해야 하며, 중복 수신 시 재처리 방지 로직을 둔다.
