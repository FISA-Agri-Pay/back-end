...
장바구니 원본 데이터는 `cart_items` 테이블에 저장한다. local/dev에서는 단일 PostgreSQL public schema를 사용하지만, 목표 구조에서는 AWS Catalog DB의 `cart_items`가 원본이 된다. AWS ElastiCache Redis는 장바구니 조회 성능 향상을 위한 캐시로 사용한다. Redis 장애 또는 캐시 미스가 발생하면 `cart_items` 테이블을 기준으로 복구한다.
