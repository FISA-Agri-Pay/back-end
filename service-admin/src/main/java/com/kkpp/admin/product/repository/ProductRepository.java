...
public interface ProductRepository extends JpaRepository<Product, Long>, JpaSpecificationExecutor<Product> {
    ...
    @Query("""
            SELECT COUNT(p)
            FROM Product p
            WHERE p.status = com.kkpp.admin.product.domain.ProductStatus.SOLD_OUT
               OR (p.status = com.kkpp.admin.product.domain.ProductStatus.ON_SALE AND p.stockQuantity <= 0)
            """)
    long countOutOfStockOrSoldOutProducts();
}
